...
//    vec = [point.x, point.y, point.z];
    ...
    return new Point(transformed_point[0], transformed_point[1], transformed_point[2]);
}
...
function draw() {
    ...
    projection_matrix = m4.perspective(Math.PI/4, 1, -1, 1);
...
//    viewport_translation[2] = 4;
    ...
    if (sort_slider.value == 1) {
        sort = true;
    } else {
        sort = false;
    }

    if (wireframe_slider.value == 1) {
        wireframe = true;
    } else {
        wireframe = false;
    }
    ...
    cube.draw(ctx, sort, wireframe, camera_matrix, projection_matrix, viewport_matrix);
